...
module.exports.getAllStudentFromCourse= function(req,res)
{
		
		console.log("fires");
		var course_id=req.query.course_id;
		//console.log(course_id);
		var sql = "select s.student_id,student_name from student as s join course_enrollment as ce on s.student_id=ce.student_id where ce.course_id ='"+course_id+"'";
		...
		var data=[];
			con.query(sql, function(error,rows)
			{
				...
				if(!error && rows.length>0)
			    {
					...
					{
					    data.push({student_id:rows[i].student_id,student_name:rows[i].student_name,cname:req.query.course_id});							
				    }
